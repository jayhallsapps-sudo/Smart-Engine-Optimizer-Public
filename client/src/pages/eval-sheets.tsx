...
const RAW_METRICS = [
  ...
  { key: "aiVisibilityScore",    label: "AI Visibility Score",source: "semrush",    type: "number", width: 110, refreshable: false, tooltip: "Enter manually from SEMrush AI Overviews report. SEMrush does not expose this via their API." },
  { key: "aiMentions",           label: "Mentioned Responses",source: "semrush",    type: "number", width: 120, refreshable: false, tooltip: "Enter manually from SEMrush AI Overviews report. SEMrush does not expose this via their API." },
  { key: "citedSources",         label: "Cited Sources",      source: "semrush",    type: "number", width: 105, refreshable: false, tooltip: "Enter manually from SEMrush AI Overviews report. SEMrush does not expose this via their API." },
...
function KeywordGapTab({ batch }: { batch: EvalBatch }) {
  ...
  async function loadGap() {
    ...
    setError(null);
    try {
      const r = await fetch(`/api/eval-batches/${batch.id}/keyword-gap`, { credentials: "include" });
      if (!r.ok) throw new Error(`${r.status}: ${r.statusText}`);
