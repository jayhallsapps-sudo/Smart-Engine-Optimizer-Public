...
function syncUrlParams(params: Record<string, string | null>) {
...
export default function BiweeklyPage() {
  ...
  const generateMut = useMutation({
    ...
    onSuccess: (data) => {
      setReport(data);
      const preservedEdits = editsRef.current;
      reportSave.setSavedReportId(null);
      syncUrlParams({ load: null });
      const meta = {
      ...
      reportSave.pendingPayloadRef.current = { reportData: data, edits: preservedEdits, meta };
      reportSave.save(data, preservedEdits, meta);
      const hasEdits = Object.keys(preservedEdits).length > 0;
      toast({
        title: "Report generated",
        description: hasEdits
          ? "Your manual edits have been preserved. Click any text to edit."
  ...
  useEffect(() => {
    if (!clientId) return;
    if (autoGeneratedRef.current === clientId) return;
    ...
    if (savedId) {
      ...
      import("@/lib/queryClient").then(({ apiRequest }) =>
        apiRequest("GET", `/api/saved-reports/${savedId}`)
          .then(r => r.json())
          .then(saved => {
            const savedEditsFromUrl = (saved.editsJson as Record<string, string>) ?? {};
            setReport(saved.generatedReportJson);
            setEdits(savedEditsFromUrl);
            editsRef.current = savedEditsFromUrl;
            reportSave.setSavedReportId(saved.id);
            reportSave.pendingPayloadRef.current = {
              reportData: saved.generatedReportJson,
              edits: (saved.editsJson as Record<string, string>) ?? {},
              meta: {
      ...
      <div className="w-64 shrink-0 border-r bg-card flex flex-col overflow-y-auto">
        ...
        <div className="flex-1 p-4 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Select value={clientId} onValueChange={(v) => { setClientId(v); setReport(null); setEdits({}); editsRef.current = {}; reportSave.setSavedReportId(null); autoGeneratedRef.current = ""; syncUrlParams({ client: v, load: null }); }}>
