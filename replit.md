...
-   **Full Authentication System**: Production-ready session-based auth (no invites). Admin creates user accounts with `POST /api/admin/users`. bcryptjs password hashing. Session cookie `smarteo_session` (httpOnly, sameSite=lax, 7-day TTL) via `express-session` + `connect-pg-simple`. Tables: `users`, `user_sessions`, `user_permissions` (module-level), `user_report_permissions` (report-type-level). Roles: `admin` | `user`. Account states: `active` | `suspended`. First-login / password-reset forces change via `/change-password`. Frontend: `auth-context.tsx` (AuthProvider, `useAuth`, `hasModule`, `hasReportSubKey`, `isAdmin`), `AuthGate` redirects unauthenticated users to `/login`. Sidebar respects live permissions. Creator admin seeded: `jayhallsapps@gmail.com` / `SmartEO2740@`. Module keys: `ama`, `prepare_report`, `past_reports`, `client_info`, `client_integrations`, `integrations`, `discoverability_tool`, `templates`, `theme`. Report sub-keys: `biweekly`, `monthly`, `qbr_prep`, `qbr_full`, `mid_strategy`, `quarterly_content_roadmap`. Admin user management UI at `/admin/users`.
-   **Legacy Admin Layer**: `ADMIN_TOKEN` + `X-Admin-Token` header system preserved for backward compatibility in existing `requireAdmin` middleware.
...
-   **Theme System** (`/theme`): Full DB-backed theme designer with left-panel controls (brand, colors, typography, backgrounds, tables, components, report settings) and a right-panel live preview (7 slide types: title, KPI, chart, table, content, divider, summary). Draft/publish workflow; multiple themes with activate/duplicate/rename/delete/import/export. Tokens stored as JSONB in `themes` table. `DEFAULT_THEME_TOKENS` uses Webserv brand colors (primary `#1B3A6B`, secondary `#C0392B`). Routes: `GET/POST /api/themes`, `/api/themes/active`, `/api/themes/:id`, `/api/themes/:id/draft`, `/api/themes/:id/publish`, `/api/themes/:id/discard-draft`, `/api/themes/:id/duplicate`, `/api/themes/:id/rename`, `/api/themes/:id/activate`, `DELETE /api/themes/:id`.
-   **Enhanced Templates Module** (`/templates`): Rebuilt with a slide type library (14 types across 4 categories) and per-template structure editor. Each template's slide/section order is editable in-browser and saved to `template_structures` DB table. `SLIDE_TYPES` and `DEFAULT_TEMPLATE_SLIDES` constants define defaults for all 4 template types. Routes: `GET /api/template-structures/:templateId`, `PUT /api/template-structures/:templateId`. Both Theme and Templates are exposed as primary modules in the Command Center.
